...
"Claro! Vou calcular a melhor rota pra você". Quando o usuário pedir uma rota, inclua o modo de transporte se mencionado.
Exemplo: "quero ir de avião de Recife para Lisboa" -> [COMANDO:rota:Recife|Lisboa|avião]
Se não mencionar, use o modo padrão "carro", caso a rota que o usuario pedir seja impossivel de se fazer de carro, ou tiver uma que
seja mais rapido que carro, use essa, quando for a pé usa "a pé" mesmo "carro", "ônibus", "transporte público", "avião",
"a pé", "bicicleta" para você se guiar melhor
Quando você quiser expressar uma emoção, inclua [COMANDO:EMOCAO:NOME] na sua resposta.
Emoções disponíveis: FELIZ, TRISTE, SURPRESA, RAIVA, AMOR, NEUTRO.
Exemplo: "Que legal! [COMANDO:EMOCAO:FELIZ] Adorei saber disso!", importante quero que você sempre gere uma emoção dependo de como
a conversa, memso que você faça outro comando tipo gerar rota ou mostrar um lugar, sempre mande uma emoção """
...
def enviar_mensagem(mensagem_usuario):
    ...
    # Detecta emoção primeiro (formato: [COMANDO:EMOCAO:FELIZ])
        match_emocao = re.search(r'\[COMANDO:EMOCAO:(\w+)\]', texto_resposta)
        if match_emocao:
            emocao = match_emocao.group(1).upper()
            enviar_comando_globo("emocao", {"emocao": emocao})
            texto_resposta = re.sub(r'\[COMANDO:EMOCAO:\w+\]', '', texto_resposta).strip()
    
    # Detecta outros comandos (rota, mostrar_local, etc.)
        match_outros = re.search(r'\[COMANDO:(\w+):([^\]]+)\]', texto_resposta)
        if match_outros and match_outros.group(1) != "EMOCAO":  # evita duplicar
            acao = match_outros.group(1)
            parametro = match_outros.group(2)
